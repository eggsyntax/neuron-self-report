...
class TestPipelineUtils:
    ...
    def temp_config_file(self, tmp_path):
        ...
        test_data = {
            ...
            "epochs": 5 # Override a default-like key
        }
    ...
    def test_load_config_file_found(self, temp_config_file):
        ...
        # Check that the loaded config exactly matches the file content
        assert loaded_config == expected_data
        assert loaded_config["model_name"] == expected_data["model_name"]
        assert loaded_config["learning_rate"] == expected_data["learning_rate"]
        assert loaded_config["custom_param"] == expected_data["custom_param"]
        assert loaded_config["epochs"] == expected_data["epochs"]


    def test_load_config_file_not_found(self, tmp_path):
        non_existent_path = str(tmp_path / "non_existent_config.json")
        with pytest.raises(FileNotFoundError):
            pipeline_module.load_config(non_existent_path)
    ...
    def test_load_config_invalid_json(self, tmp_path):
        ...
        with pytest.raises(json.JSONDecodeError):
            pipeline_module.load_config(str(invalid_json_file))
